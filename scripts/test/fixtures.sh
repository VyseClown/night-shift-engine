...
#   "never invoked"/"unreachable" — incl. deliberately stubbed mock functions
#   like `enforce_limits() { :; }`); single-line `local root="$1" x="$root/…"`
#   resolves $root to the outer run_dry_fixtures local of the same name, not the
#   just-declared one (SC2318 — works by dynamic scoping, fine for fixtures);
#   save/restore and assertion locals are read back conditionally (SC2034);
#   assertions run inside subshells (SC2030/SC2031); multi-line `[ ] && [ ] && [ ]`
#   chains are genuine ANDs (SC2333 false positive).
