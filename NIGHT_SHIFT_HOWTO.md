...
3. One-time pre-flight (proves the toolchain + Claude can run unattended):
   ...
   ```
4. Launch from a terminal, in the `night-shift-engine/` directory (not a Claude session):
   ```sh
   NIGHT_SHIFT_ACCEPT_COSTS=YES scripts/night-shift.sh \
     --project <workspace>/web-app --spec specs/my-feature.md
   ```
   Omit `--spec` to let it pull the next `TODO.md` entry.
5. **Or launch from the viewer** (a UI over the same script — no terminal):
   ```sh
   cd <workspace>/night-shift-viewer/server && NSV_ALLOW_EDIT=1 NSV_ALLOW_LAUNCH=1 NSV_ALLOW_REAL=1 npm run dev:real
   cd <workspace>/night-shift-viewer/web && npm run dev      # second terminal
   ```
   ...
   *Real run* → pick the project + spec → watch it stream. The project must be on
   ...
   refuses otherwise). The viewer **auto-discovers** target repos in the workspace container:
